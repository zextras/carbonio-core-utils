...
my $ldifin;
my $ldifout;
my $zimbra_ldap_userdn;
my $zimbra_server_hostname;
my $infile;
my $outfile;

my $zimbra_ldap_password;
...
sub preLdapStart {
  ...
  $zimbra_ldap_userdn=getLocalConfig("zimbra_ldap_userdn");
  $zimbra_ldap_password=getLocalConfig("zimbra_ldap_password");
  $ldap_root_password=getLocalConfig("ldap_root_password");
  $zimbra_server_hostname=getLocalConfig("zimbra_server_hostname");
  ...
  $outfile="$zimbra_tmp_directory/olcDatabase\=\{0\}config.ldif.$$";
...
sub setLocalConfigRandom {
  my $key = shift;
  qx(su - zextras -c '/opt/zextras/bin/zmlocalconfig -f -e -r ${key} 2> /dev/null');
}
